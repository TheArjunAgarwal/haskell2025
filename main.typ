...
```haskell  
  fib :: Integer -> Integer
  fib n = fibl 0 1 !! n where
    fibl a b = a : fibl (a+b) a
```
The given regex will capture the Haskell Code and will be used to build the pre-processor to make the .lhs file!
```regex
\`\`\`haskell([^\`]+)\`\`\`
```
...
#idea(title: [How to find an anti-gradient with two variables])[
...
#idea(title: [How to find an anti-gradient with three variables])[
  1. Let $f$ denote the gradient function.
  2. Integrate the given $(partial f) / (partial x)$ with respect to $x$ to get some equation of the
...
#side(title: [Side Note: There's a huge difference
